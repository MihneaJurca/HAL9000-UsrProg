...
typedef struct _PROCESS_SYSTEM_DATA
{
    ...
    //Map handle to process
    LIST_ENTRY      UM_HandleMapProcess;
    ...
    )
{
    ...
    InitializeListHead(&m_processData.UM_HandleMapProcess);
    m_processData.HandleValue = 3;
...
PLIST_ENTRY
getUM_HandleMapProcess(
    void
) 
{
    return &m_processData.UM_HandleMapProcess;
}

UM_HANDLE
getUM_HandleValueProcess(
    void
)
{
    return m_processData.HandleValue;
}
